...
interface AnnoService {
    ...
    //ok 没有searchMap的话返回所有数据，否则根据传递的map筛选
    //搜索问题
    ...
    fun getAnswer(@Path("type") type: String,
                  @Query("question_id") question_id: Int): Deferred<CommonBody<ReplyOrCommit>>

    //ok 成功点赞或取消返回null，只用解析错误信息
    //点赞/取消点赞 问题/评论/回复
    //http://47.93.253.240:10805/api/user/question(answer/commit)/like(dislike)
    @JvmSuppressWildcards
    @FormUrlEncoded
    @POST("{type}/{isLike}")
    fun postThumbUp(@Path("type") type: String,
                    @Path("isLike") isLike: String,
                    @Field("id") question_id: Int,
                    @Field("user_id") user_id: Int): Deferred<CommonBody<IsLike>>

    //ok 成功添加问题返回null，只用解析错误信息
    //添加问题
    //http://47.93.253.240:10805/api/user/question/add
    @FormUrlEncoded
    @POST("question/add")
    fun addQuestion(@Field("user_id") user_id: Int,
                    @Field("name") name: String,
                    @Field("description") description: String,
                    @Field("tagList") tagList: List<Int>): Deferred<AddQuestion>

    //ok 成功提交返回null，只用解析错误信息
    //评价回复 (给官方回复打分)
    ...
    //这个地方有问题，成功返回列表，错误返回map
    //获得已点赞的问题/评论/回复列表
    //http://47.93.253.240:10805/api/user/likes/get/question(commit/answer)?user_id=1
    @GET("likes/get/{type}")
    fun getLikedQCA(@Path("type") type: String,
                    @Query("user_id") user_id: Int): Deferred<CommonBody<List<Liked>>>

    //ok 成功返回true或false，否则返回错误信息
    //获取问题/评论/回复的点赞状态
    //http://47.93.253.240:10805/api/user/likes/get/isLiked/question?user_id=1&id=1    id:问题/评论/回复的id
