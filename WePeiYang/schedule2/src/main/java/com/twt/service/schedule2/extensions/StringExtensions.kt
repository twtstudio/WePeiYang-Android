...
fun span(s: CharSequence, spanStyle: Any) = (if (s is String) SpannableString(s) else s as? SpannableString
        ?: SpannableString("")).apply { setSpan(spanStyle, 0, length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE) }

fun span(s: CharSequence, spanStyleList: List<Any>) = (if (s is String) SpannableString(s) else s as? SpannableString
