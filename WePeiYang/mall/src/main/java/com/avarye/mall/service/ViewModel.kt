...
class ViewModel {
     ...
     * 先后台登陆再获得最新发布数据
     */
    fun init(fragment: MallSaleFragment) = GlobalScope.launch(Dispatchers.Main) {
        ...
        loadingLiveData.postValue(false)
        fragment.srl_main_sale.isRefreshing = false
    }

    fun getLatestSale(page: Int) = GlobalScope.launch(Dispatchers.Main) {
    ...
    fun getLatestNeed(page: Int) = GlobalScope.launch(Dispatchers.Main) {
     ...
     * 通过分类菜单获得筛选数据
     */
    fun getSelect(category: String, which: Int, page: Int) = GlobalScope.launch(Dispatchers.Main) {
    ...
    fun getSellerSale(id: String, token: String) = GlobalScope.launch(Dispatchers.Main) {
    ...
    fun getSellerNeed(id: String, token: String) = GlobalScope.launch(Dispatchers.Main) {
...
//                    imgIdLiveData.postValue(it.id)
    ...
    fun postSale(map: Map<String, Any>, token: String, activity: PostActivity) = GlobalScope.launch(Dispatchers.Main) {
    ...
    fun postNeed(map: Map<String, Any>, token: String, uid: String, activity: PostActivity) = GlobalScope.launch(Dispatchers.Main) {
    ...
    fun fav(id: String, token: String, activity: DetailActivity) = GlobalScope.launch(Dispatchers.Main) {
     ...
     * 获得我的发布列表、我的需求列表
     */
    fun getMyList(id: String, which: Int) = GlobalScope.launch(Dispatchers.Main) {
     ...
     * 获得我的收藏列表
     */
    fun getFavList(token: String) = GlobalScope.launch(Dispatchers.Main) {
     ...
     * 删除发布商品
     */
    fun deleteSale(gid: String, token: String, uid: String) = GlobalScope.launch(Dispatchers.Main) {
     ...
     * 删除发布需求
     */
    fun deleteNeed(nid: String, token: String, uid: String) = GlobalScope.launch(Dispatchers.Main) {
     ...
     * ListActivity里的取消收藏
     */
    fun deFav(gid: String, token: String) = GlobalScope.launch(Dispatchers.Main) {
     ...
     */
    fun deFav(gid: String, token: String, activity: DetailActivity) = GlobalScope.launch(Dispatchers.Main) {
        MallManager.deFavAsync(gid, token).awaitAndHandle {
        }?.let {
            when (it.result_code) {
                "00021" -> {
     ...
     * 修改个人信息
     */
    fun changeMyInfo(phone: String, email: String, qq: String, campus: Int) = GlobalScope.launch(Dispatchers.Main) {
